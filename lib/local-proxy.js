...
function proxyLocalFiles (req, res, next) {
  ...
  if (cleanedPath.match(regexMashup) && !cleanedPath.match(/\.map$/)) {
    ...
    let data = ''
    // is res.on part of connect or expres, not available here?
    // res.listenerCount() is 0
    // https://stackoverflow.com/questions/6246052/http-server-doesnt-have-addlistener-in-node-js-it-isnt-a-event-emitter
    res.on('data', (chunk) => {
      data += chunk
    })
    res.once('end', () => {
