...
const argv = yargs
  ...
  .options({
    ...
    d: {
      alias: 'directory',
      describe:
        'Path to the directory where local files are stored. Default: current directory',
      type: 'string',
    },
...
const directory = (argv.directory || config.directory || './').replace(
  /\/\/$/,
  '',
...
const open =
  (typeof argv.open !== 'undefined' && argv.open) || config.open || false
const noOpen =
  (typeof argv.noOpen !== 'undefined' && argv.noOpen) || config.noOpen
...
logger.info(
  '\n{yellow:%s} {magenta:%s}',
  `${protocol.toUpperCase()} protocol was ${
    isProtocolInferred ? 'inferred' : 'set'
  } - ensure you develop on:`,
...
logger.info(
  '{bold:Found }{cyan:%s}{bold: files to serve:}',
  filesToWatch.length,
)
...
async function proxyLocalFiles(req, res, next) {
  const url = new URL(target + req.url)
  const { pathname } = url

  if (pathname.match(regexMashup)) {
    // strip the prefix out of `pathname`
    const localFileName = path.join(directory, pathname.replace(prefix, ''))
    if (CHATTY) logger.info('Match: %s', pathname)
    ...
    try {
      ...
      logger.info(
        '{red:404 %s} {white:%s}',
        localFileName,
        '(local-proxy -v for more info)',
      )
      // ...continue with normal broswer-sync response
      next()
    }
  } else {
    // On first load, sub-fetch the root URL of the proxied target
    // to ensure that we're configured correctly
    if (isFirstLoad && pathname === '/') {
      ...
      try {
        ...
        if (!originResponse.ok)
          throw new Error(`Failed to fetch proxied URL ${url.toString()}`)

        const body = await originResponse.text()
        const [_matches, title] = body.match(/<title>(.*)<\/title>/)
        if (title) logger.info('{green:%s}', `\nFound page: ${title}`)
        else
          logger.info(
            '{yellow:%s}',
            `\nGot a response, but is this the correct URL? ${url.toString()}`,
          )

        logger.info('{lightgrey:%s} {grey:%s}', res.statusCode, req.url)
        next()
      } catch (error) {
        if (error.message.includes('ENOTFOUND'))
          logger.info(
            '{red:%s}',
            `\nError finding ${target}/ - please check the target URL!\n\n> ${error.message}`,
          )
        else {
          logger.info('{red:%s}', error.message)
        }
        if (CHATTY) console.error(error)
        process.exit(1)
      }
    } else {
      if (DEBUG)
